...
def scatter_images(input_folder, output_folder, num_images_to_generate):
    ...
    for i in range(num_images_to_generate):
        ...
        if num_images_used == 1:
            ...
            density = 1
        elif num_images_used == 4:
            density = .8
        elif num_images_used == 5:
            density = .6
        else:
            density = .3
        ...
        for image_file in selected_files:
            ...
            for _ in range(repetitions):
                ...
                random_size = random.uniform(0.7, 8)
                ...
                # Randomly apply a skew effect
                if random.random() < 0.5:
                    image_copy = image_copy.transform(image_copy.size, Image.AFFINE, (1, random.uniform(0.1, 0.5), random.uniform(0.1, 0.5), 0, 1, 0))

                # Randomize transparency
                alpha = image_copy.split()[3]  # Get the alpha channel
                alpha = ImageEnhance.Brightness(alpha).enhance(random.uniform(0.4, 1))  # Random transparency
